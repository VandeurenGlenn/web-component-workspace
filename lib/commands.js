...
function bowerDeps(bowerJson) {
  ...
  if (bowerjson.dependencies) {
    Object.keys(bowerjson.dependencies).forEach(function (dep){
      deps.push(bowerjson.dependencies[dep]);
    });
  }
  return dependencies;
}
...
Commands.prototype = {
  ...
  deps: function(dir) {
    return this._db.hasPackage(dir).then(function(hasPackage){
      console.log(path.join(this._workdir, dir));
      var bowerJson = readBower(path.join(this._workdir, dir), component);
    ...
    resolved.forEach(function(pkg){
      ...
      if (pkg.repo) {
        ...
        if (!shell.test('-e', pkg.folder) ||
          ...
          shell.cp('-R', pkg.sourcedir, pkg.folder); 
  ...
  update: function() {
    this._db.getComponents().then(function(component){
